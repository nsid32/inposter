...
function PostCard({ post, onStatusChange, onContentChange, onPublish, onImageChange, onDelete, onResend, hasUnsplashKey }: PostCardProps) {
  ...
  return (
    <>
      <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 space-y-3 hover:border-slate-700 transition-colors">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-slate-600">{post.content.length} chars</span>
        </div>
        ...
        {post.imageData ? (
          <div className="relative inline-block">
            <img
              ...
              alt="Post image"
              className="rounded-lg max-h-40 max-w-full object-cover border border-slate-700"
            />
            <button
              type="button"
          ...
          (post.status === "draft" || post.status === "approved") && (
            <div className="flex gap-2 flex-wrap">
              <button
                ...
                disabled={imageLoading}
                className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-300 border border-dashed border-slate-700 rounded-lg px-2.5 py-1.5 hover:border-slate-600 transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-300 border border-dashed border-slate-700 rounded-lg px-2.5 py-1.5 hover:border-slate-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
        ...
        {post.status === "published" && (
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs text-slate-500">Published {formatDate(post.publishedAt)}</span>
            {post.linkedinUrl && (
              <a
        ...
        <DialogContent className="bg-slate-900 border-slate-800 text-white max-w-2xl">
          ...
          <div className="space-y-3">
            <Textarea
              ...
              className="bg-slate-800 border-slate-700 text-slate-200 min-h-[200px]"
            />
...
const COLUMNS: Column[] = [
  { label: "Drafts", status: "draft", headerColor: "border border-slate-700 bg-slate-900 border-l-4 border-l-slate-500", emptyMessage: "No drafts yet" },
  { label: "Approved", status: "approved", headerColor: "border border-slate-700 bg-slate-900 border-l-4 border-l-blue-500", emptyMessage: "No approved posts" },
  { label: "Published", status: "published", headerColor: "border border-slate-700 bg-slate-900 border-l-4 border-l-emerald-500", emptyMessage: "No published posts" },
];
    ...
    <>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Manage your AI-generated posts</p>
          </div>
        </div>
          ...
          {COLUMNS.map((col) => {
            ...
            return (
              <div key={col.status} className="space-y-3">
                ...
                {/* Cards */}
                {colPosts.length === 0 ? (
                  <div className="rounded-xl border border-slate-800 border-dashed flex items-center justify-center py-8 text-center px-3">
                    <p className="text-xs text-slate-600">{col.emptyMessage}</p>
                  </div>
                ) : (
                  colPosts.map((post) => (
                    <PostCard
      ...
      <AlertDialog open={!!publishTarget} onOpenChange={(open) => { if (!open) setPublishTarget(null); }}>
        <AlertDialogContent className="bg-slate-900 border-slate-800 text-white">
          ...
          {publishTarget && (
            <div className="rounded-lg border border-slate-700 bg-slate-800 p-3 my-2">
              <p className="text-sm text-slate-200 leading-relaxed line-clamp-6">
                {publishTarget.content}
              </p>
              <p className="text-xs text-slate-500 mt-2">{publishTarget.content.length} characters</p>
            </div>
          )}
      ...
      <AlertDialog open={deleteTarget !== null} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }}>
        <AlertDialogContent className="bg-slate-900 border-slate-800 text-white">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteTarget(null)} className="border-slate-700 text-slate-300 hover:bg-slate-800 bg-transparent">
              Cancel
            </AlertDialogCancel>
      ...
      <AlertDialog open={!!resendTarget} onOpenChange={(open) => { if (!open) setResendTarget(null); }}>
        <AlertDialogContent className="bg-slate-900 border-slate-800 text-white">
          ...
          {resendTarget && (
            <div className="rounded-lg border border-slate-700 bg-slate-800 p-3 my-2">
              <p className="text-sm text-slate-200 leading-relaxed line-clamp-4">{resendTarget.content}</p>
            </div>
          )}
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setResendTarget(null)} className="border-slate-700 text-slate-300 hover:bg-slate-800 bg-transparent">
              Cancel
            </AlertDialogCancel>
